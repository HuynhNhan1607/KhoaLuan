...
// Trajectory JSON accumulation buffer (separate from other messages)
#define TRAJ_BUFFER_SIZE 16384 // 16KB for large trajectories
static char g_traj_buffer[TRAJ_BUFFER_SIZE];
static size_t g_traj_buffer_len = 0;
static bool g_accumulating_traj = false;
      ...
      if (FD_ISSET(i, &working_set))
      {
        ...
        else
        {
          ...
          else
          {
            ...
            {
              // === TRAJECTORY ACCUMULATION MODE ===
              if (is_traj_start && !g_accumulating_traj)
              {
                // Start new accumulation
                g_traj_buffer_len = 0;
                g_accumulating_traj = true;
                printf("[SOCKET] Starting trajectory accumulation...\n");
              }

              // Append to trajectory buffer
              size_t space_left = TRAJ_BUFFER_SIZE - g_traj_buffer_len - 1;
              size_t copy_len =
                  (size_t)valread < space_left ? (size_t)valread : space_left;
              memcpy(g_traj_buffer + g_traj_buffer_len, buffer, copy_len);
              g_traj_buffer_len += copy_len;
              g_traj_buffer[g_traj_buffer_len] = '\0';

              // Find end of trajectory JSON: look for "}}" pattern
              // This handles cases where another message follows in same packet
              char *traj_end = NULL;
              for (size_t pos = 1; pos < g_traj_buffer_len; pos++)
              {
                if (g_traj_buffer[pos] == '}' &&
                    g_traj_buffer[pos - 1] == '}')
                {
                  // Check if this is followed by end of string, newline, or new
                  // JSON
                  char next_char = g_traj_buffer[pos + 1];
                  if (next_char == '\0' || next_char == '\n' ||
                      next_char == '\r' || next_char == '{')
                  {
                    traj_end = &g_traj_buffer[pos];
                    break;
                  }
                }
              }
              ...
              if (traj_end != NULL)
              {
                // Found complete trajectory JSON
                size_t traj_len = (size_t)(traj_end - g_traj_buffer + 1);
                char saved_char = g_traj_buffer[traj_len];
                g_traj_buffer[traj_len] = '\0'; // Temporarily null-terminate

                printf("[SOCKET] Trajectory JSON complete (%zu bytes), "
                       "parsing...\n",
                       traj_len);
                if (trajectory_load(g_traj_buffer))
                {
                  printf("[SOCKET] Trajectory loaded successfully\n");
                  printf("[SOCKET] Waiting for 'execute_trajectory' command "
                         "with 'time' field\n");
                }
                else
                {
                  printf("[SOCKET] Failed to load trajectory\n");
                }

                g_traj_buffer[traj_len] = saved_char; // Restore

                // Check if there's leftover data (another message after
                // trajectory)
                if (traj_len < g_traj_buffer_len)
                {
                  char *leftover = &g_traj_buffer[traj_len];
                  // Skip any whitespace/newlines between messages
                  while (*leftover == '\n' || *leftover == '\r' ||
                         *leftover == ' ')
                  {
                    leftover++;
                  ...
                  if (*leftover != '\0')
                  {
                    printf("[SOCKET] Processing leftover data: %s\n", leftover);

                    // Check if leftover contains execute_trajectory command
                    if (strstr(leftover, "\"cmd\":\"execute_trajectory\"") ||
                        strstr(leftover, "\"cmd\": \"execute_trajectory\""))
                    {
                      printf("[SOCKET] Found execute_trajectory in leftover, "
                             "processing...\n");

                      // Parse MANDATORY time field
                      char *time_ptr = strstr(leftover, "\"time\":");
                      if (time_ptr)
                      {
                        time_ptr += 7; // Skip "\"time\":"
                        while (*time_ptr == ' ')
                          time_ptr++; // Skip whitespace
                        double start_time = strtod(time_ptr, NULL);

                        if (start_time > 0.0)
                        {
                          printf("[SOCKET] Scheduling trajectory start at "
                                 "epoch %.3f\n",
                                 start_time);
                          trajectory_start_at(start_time);
                        }
                        else
                        {
                          printf("[SOCKET] ERROR: Invalid time value (%.3f). "
                      ...
                      {
                        printf("[SOCKET] ERROR: Missing 'time' field in "
                               "execute_trajectory! Trajectory NOT started!\n");
                      }
                    }
                    else
                    {
                      ...
                      client_manager_broadcast(leftover, leftover_len);
                    }
                  }
                }

                // Reset accumulation
                g_accumulating_traj = false;
                g_traj_buffer_len = 0;
              }
              else
              {
                printf(
                    "[SOCKET] Accumulating trajectory... (%zu bytes so far)\n",
                    g_traj_buffer_len);
              }
            }
            // === NON-TRAJECTORY MESSAGES (pass through as before) ===
            else if (strstr(buffer, "\"type\":\"control\"") ||
                     strstr(buffer, "\"type\": \"control\""))
            {
              // Check for execute_trajectory command
              if (strstr(buffer, "\"cmd\":\"execute_trajectory\"") ||
                  strstr(buffer, "\"cmd\": \"execute_trajectory\""))
              {
                printf("[SOCKET] Received execute_trajectory command\n");
                ...
                if (time_ptr)
                {
                  ...
                  {
                    printf(
                        "[SOCKET] Scheduling trajectory start at epoch %.3f\n",
                        start_time);
                    trajectory_start_at(start_time);
                  }
                  else
              ...
              // Check for lock_transport_offset command (Phase 2)
              else if (strstr(buffer, "\"cmd\":\"lock_transport_offset\"") ||
                       strstr(buffer, "\"cmd\": \"lock_transport_offset\""))
              {
                printf("[SOCKET] Received lock_transport_offset command\n");
              ...
              else
              {
                ...
                printf(
                    "[SOCKET] Control command from laptop (line 651): %.80s\n",
                    buffer);
                // formation_dispatch_user_control();
              }
            }
            else if (strstr(buffer, "Upgrade"))
            {
              pthread_mutex_lock(&g_firmware_mutex);
              ...
              if (!already_updating)
              {
                ...
                // Forward Upgrade to ESP32
                client_manager_broadcast("Upgrade", strlen("Upgrade"));
                printf("[LAPTOP] Sent 'Upgrade' command to ESP32\n");
            ...
            {
              // CRITICAL: Emergency stop - immediately halt all motion
              printf("[LAPTOP] *** EMERGENCY STOP COMMAND RECEIVED ***\n");
              trajectory_stop();
            }
            ...
            // Only process non-trajectory messages here
            if (!g_accumulating_traj)
            {
              if (strstr(buffer, "arm_ik_request") ||
                  strstr(buffer, "arm_servo_cmd") ||
                  strstr(buffer, "arm_pick") || strstr(buffer, "arm_place") ||
              ...
              else
              {
                ...
                // Detect user manual control (raw dot_x or other commands)
              }
            }
          }
        }
