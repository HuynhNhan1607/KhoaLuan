...
void PID_Init(PID_Context_t *ctx, float kp, float ki, float kd, float output_limit)
{
...
float PID_Compute(PID_Context_t *ctx, float setpoint, float measurement, float dt)
{
    if (dt <= 0.0f || dt > 1.0f)
    {
    ...
    ctx->filtered_derivative += alpha_d * (derivative_raw - ctx->filtered_derivative);
...
void PID_StartAutotune(PID_Context_t *ctx, float relay_step)
{
    ...
    ctx->rpm_peak_max = 0.0f;
    ctx->rpm_peak_min = 0.0f;
    ctx->first_cycle = true;
    memset(ctx->crossing_times_ms, 0, sizeof(ctx->crossing_times_ms));
}

float PID_RunAutotune(PID_Context_t *ctx, float target, float measurement, uint32_t current_time_ms)
{
    ...
    // Track peaks
    if (measurement > ctx->rpm_peak_max)
        ctx->rpm_peak_max = measurement;
    if (measurement < ctx->rpm_peak_min || ctx->first_cycle)
        ctx->rpm_peak_min = measurement;

    // Detect zero crossings
    if (ctx->last_error_sign != 0 && error_sign != 0 && ctx->last_error_sign != error_sign)
    {
    ...
    // Nếu đơn vị là Rad/s, 0.5f có thể hơi lớn, nhưng tạm giữ nguyên hoặc giảm xuống 0.1f nếu cần
...
//     for (int i = 2; i < ctx->crossing_count && i < PID_AUTOTUNE_MAX_CROSSINGS; i++)
//     {
//         uint32_t period_ms = ctx->crossing_times_ms[i] - ctx->crossing_times_ms[i - 2];
...
//     ctx->Ku = (rpm_amplitude > 0.1f) ? ((4.0f * d) / (3.14159f * rpm_amplitude)) : 0.0f;
...
void PID_ApplyAutotuneResults(PID_Context_t *ctx)
{
    ...
    // --- Công thức cải tiến: Giảm Ki và thêm lại Kd ---
    if (ctx->Tu > 0.0f && ctx->Ku > 0.0f)
    {
        // 1. Kp: Giữ nguyên Tyreus-Luyben nhưng thêm hệ số an toàn
        float Kp_safe = ctx->Ku / 3.5f; // Giảm từ 3.2 xuống 3.5 để êm hơn

        // 2. Ki: GIẢM MẠNH để tránh tích lũy quá nhanh
        // Công thức cũ: Ti = 2.2 * Tu -> Ki = Kp / Ti
        // Công thức mới: Ti = 4.0 * Tu (tăng gấp đôi thời gian tích phân)
        float Ki_safe = Kp_safe / (4.0f * ctx->Tu); // Giảm Ki xuống ~1/2

        // 3. Kd: Thêm lại với giá trị nhỏ để cải thiện khả năng dừng
        // Td = Tu / 8 (nhỏ để không khuếch đại nhiễu)
        float Kd_safe = Kp_safe * (ctx->Tu / 8.0f);
...
void PID_SetMotorCompensation(PID_Context_t *ctx, float deadband, float static_ff)
{
