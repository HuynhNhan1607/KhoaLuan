...
static void set_motor_pwm(uint8_t motor_index, float duty_cycle)
{
    ...
    // Set PWM duty cycle
    mcpwm_comparator_set_compare_value(motors[motor_index].comparator, compare_value);

    if (duty_cycle > 0)
    {
        // Forward: L_PWM active, R_PWM low
        mcpwm_generator_set_force_level(motors[motor_index].generator_r, MCPWM_FORCE_LOCK_LOW, true);   // Force R low
        mcpwm_generator_set_force_level(motors[motor_index].generator_l, MCPWM_FORCE_ENABLE_PWM, true); // Enable L PWM
    }
    else if (duty_cycle < 0)
    {
        // Reverse: L_PWM low, R_PWM active
        mcpwm_generator_set_force_level(motors[motor_index].generator_l, MCPWM_FORCE_LOCK_LOW, true);   // Force L low
        mcpwm_generator_set_force_level(motors[motor_index].generator_r, MCPWM_FORCE_ENABLE_PWM, true); // Enable R PWM
    }
    else
    {
        // Stop: Both low
        mcpwm_generator_set_force_level(motors[motor_index].generator_l, MCPWM_FORCE_LOCK_LOW, true);
        mcpwm_generator_set_force_level(motors[motor_index].generator_r, MCPWM_FORCE_LOCK_LOW, true);
    }
}
...
void motor_driver_init(void)
{
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        if (ret != ESP_OK)
        {
            ESP_LOGE(TAG, "Failed to create Shared Timer %d: %s", i, esp_err_to_name(ret));
            return;
        }
    }
    ...
    for (int i = 0; i < NUM_MOTORS; i++)
    {
        ...
        ESP_ERROR_CHECK(mcpwm_operator_connect_timer(motors[i].operator, shared_timers[group_id]));
        ...
        // 3.4. Tạo Generators
        mcpwm_generator_config_t generator_l_config = {.gen_gpio_num = motors[i].l_pwm_gpio};
        mcpwm_generator_config_t generator_r_config = {.gen_gpio_num = motors[i].r_pwm_gpio};

        ESP_ERROR_CHECK(mcpwm_new_generator(motors[i].operator, &generator_l_config, &motors[i].generator_l));
        ESP_ERROR_CHECK(mcpwm_new_generator(motors[i].operator, &generator_r_config, &motors[i].generator_r));

        // 3.5. Cấu hình Action
        ESP_ERROR_CHECK(mcpwm_generator_set_action_on_timer_event(motors[i].generator_l,
                                                                  MCPWM_GEN_TIMER_EVENT_ACTION(MCPWM_TIMER_DIRECTION_UP, MCPWM_TIMER_EVENT_EMPTY, MCPWM_GEN_ACTION_HIGH)));
        ESP_ERROR_CHECK(mcpwm_generator_set_action_on_compare_event(motors[i].generator_l,
                                                                    MCPWM_GEN_COMPARE_EVENT_ACTION(MCPWM_TIMER_DIRECTION_UP, motors[i].comparator, MCPWM_GEN_ACTION_LOW)));

        ESP_ERROR_CHECK(mcpwm_generator_set_action_on_timer_event(motors[i].generator_r,
                                                                  MCPWM_GEN_TIMER_EVENT_ACTION(MCPWM_TIMER_DIRECTION_UP, MCPWM_TIMER_EVENT_EMPTY, MCPWM_GEN_ACTION_HIGH)));
        ESP_ERROR_CHECK(mcpwm_generator_set_action_on_compare_event(motors[i].generator_r,
                                                                    MCPWM_GEN_COMPARE_EVENT_ACTION(MCPWM_TIMER_DIRECTION_UP, motors[i].comparator, MCPWM_GEN_ACTION_LOW)));

        // 3.6. Set trạng thái ban đầu an toàn (STOP)
        mcpwm_generator_set_force_level(motors[i].generator_l, MCPWM_FORCE_LOCK_LOW, true);
        mcpwm_generator_set_force_level(motors[i].generator_r, MCPWM_FORCE_LOCK_LOW, true);
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        ESP_ERROR_CHECK(mcpwm_timer_start_stop(shared_timers[i], MCPWM_TIMER_START_NO_STOP));
    }
...
void motor_control_update(void)
{
    ...
    if (autotune_active)
    {
        ...
        for (int i = 0; i < NUM_MOTORS; i++)
        {
            ...
            float relay_pwm = PID_RunAutotune(&motors[i].pid, motors[i].target_speed, motors[i].current_speed, current_time_ms);
        ...
        if (completed == NUM_MOTORS && autotune_completed_count != NUM_MOTORS)
        {
            ...
            for (int i = 0; i < NUM_MOTORS; i++)
            {
                ...
                ESP_LOGI(TAG, "Motor %d - Kp: %.3f, Ki: %.3f, Kd: %.3f, Tu: %.3f, Ku: %.3f",
    ...
    else
    {
        ...
        for (int i = 0; i < NUM_MOTORS; i++)
        {
            ...
            float speed_change = fabs(motors[i].target_speed - motors[i].prev_target_speed);
            ...
            float raw_pid_output = PID_Compute(&motors[i].pid, motors[i].target_speed, motors[i].current_speed, 0.01f);
            int compensated_pwm_int = PID_ApplyMotorCompensation(&motors[i].pid, (int)raw_pid_output);
...
{
    // Default values: 70 RPM, 100 PWM
    motor_start_autotune_custom(70.0f, 100.0f);
}

void motor_start_autotune_custom(float target_rpm, float relay_pwm)
{
    ESP_LOGI(TAG, "Starting PID autotune for all 4 motors...");

    // Safety limits: RPM max 110, PWM max 100
    if (target_rpm > 110.0f)
    {
        ESP_LOGW(TAG, "RPM %.1f exceeds limit, clamping to 110 RPM", target_rpm);
        target_rpm = 110.0f;
    }
    ...
    float target_rads = target_rpm * 0.10472f; // Convert RPM to rad/s
    ...
    ESP_LOGI(TAG, "Autotune started (Target: %.1f RPM, Relay: %.1f PWM)", target_rpm, relay_pwm);
}

bool motor_is_autotune_running(void)
{
    return autotune_active;
}
...
esp_err_t motor_save_pid_to_nvs(void)
{
    ...
    for (int i = 0; i < NUM_MOTORS; i++)
    {
        ...
        ESP_LOGI(TAG, "Saved Motor %d PID: Kp=%.3f, Ki=%.3f, Kd=%.3f",
                 i, motors[i].pid.kp, motors[i].pid.ki, motors[i].pid.kd);
    }
...
esp_err_t motor_load_pid_from_nvs(void)
{
    ...
    for (int i = 0; i < NUM_MOTORS; i++)
    {
            ...
            nvs_get_u32(nvs_handle, key_kd, &kd_bits) == ESP_OK)
        {
            ...
            // ESP_LOGW(TAG, "Motor %d PID: Kp=%.3f, Ki=%.3f, Kd=%.3f", i, kp, ki, kd);
            ESP_LOGW(TAG, "Loaded Motor %d PID: Kp=%.3f, Ki=%.3f, Kd=%.3f", i, kp, ki, kd);
        }
        else
        {
