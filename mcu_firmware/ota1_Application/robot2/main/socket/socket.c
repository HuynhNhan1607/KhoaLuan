...
static inline float rpm_to_rads(float rpm)
{
    return rpm * 0.10472f;
}

static inline float rads_to_rpm(float rads)
{
    return rads * 9.5493f;
}
...
void task_socket(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        if (len > 0)
        {
            ...
            {
                ESP_LOGW(TAG_Socket, "Received dot_x: %f, dot_y: %f, dot_theta: %f", dot_x, dot_y, dot_theta);
                // MecanumSpeedControl(theta, dot_x, dot_y, dot_theta);
                set_robot_status(dot_x, dot_y, dot_theta);
            }

            else if (sscanf(rx_buffer, "Localize: X=%f Y=%f", &odom.distance[X_AXIS], &odom.distance[Y_AXIS]) == 2)
            {
                ESP_LOGW(TAG_Socket, "Setting Position X: %.2f, Y: %.2f", odom.distance[X_AXIS], odom.distance[Y_AXIS]);
            }
            // Motor Set Speed
            else if (sscanf(rx_buffer, "MOTOR:%d SPEED:%d", &motor_id, &motor_speed) == 2)
            {
                ESP_LOGW(TAG_Socket, "Setting Motor %d Speed to %d RPM", motor_id, motor_speed);
            ...
            {
                ESP_LOGW(TAG_Socket, "Starting PID Autotune with default parameters (70 RPM, 100 PWM)...");
                motor_start_autotune();
            }
            // PID Autotune with custom parameters: RPM:xxx PWM:yyy
            else if (strncmp(rx_buffer, "RPM:", 4) == 0)
            {
                float target_rpm = 70.0f; // Default
                float relay_pwm = 100.0f; // Default
                ...
                if (sscanf(rx_buffer, "RPM:%f PWM:%f", &target_rpm, &relay_pwm) >= 1)
                {
                    ...
                    ESP_LOGW(TAG_Socket, "Starting PID Autotune with custom parameters: RPM=%.1f, PWM=%.1f", target_rpm, relay_pwm);
                    motor_start_autotune_custom(target_rpm, relay_pwm);
                }
                else
                {
                    ESP_LOGW(TAG_Socket, "Invalid RPM/PWM format. Use: RPM:<value> PWM:<value>");
                }
            }
            else
            {
...
void send_rpm_data(int socket)
{
    ...
    EstimateRobotPosition(&odom, yaw_angle, wheel_infor, (end_time - start_time) / 1000000.0f);

    // ESP_LOGW(TAG_Socket, "x: %.2fm y: %.2fm", odom.distance[X_AXIS], odom.distance[Y_AXIS]);
    // Format the JSON string with robot ID and 4 motor values
    char message[256];
#if SEND_RPM == 1
    int message_len = snprintf(message, sizeof(message),
                               "{\"id\":\"%s\",\"type\":\"encoder\",\"data\":[%.2f,%.2f,%.2f,%.2f]}\n",
                               ID_ROBOT,
                               rads_to_rpm(wheel_infor[0].WheelSpeed),
    ...
    message_len = snprintf(message, sizeof(message),
                           "{\"id\":\"%s\",\"type\":\"position\",\"source\":\"odometry\",\"data\":{\"position\":[%.4f,%.4f],\"velocity\":[%.4f,%.4f]}}\n",
                           ID_ROBOT, odom.distance[X_AXIS], odom.distance[Y_AXIS], odom.velocity[X_AXIS], odom.velocity[Y_AXIS]);
...
{
    xTaskCreatePinnedToCore(task_socket, "task_socket", 4096, &socket_server, 5, NULL, 1);
    xTaskCreatePinnedToCore(task_send_encoder, "task_send_encoder", 4096, &socket_server, 5, NULL, 1);
}