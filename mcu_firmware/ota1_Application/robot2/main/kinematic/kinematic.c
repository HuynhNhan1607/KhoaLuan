                         ...
                         float lateral_velocity, float longitudinal_velocity, float yaw_speed)
{
    ...
    const float velocity_component_w13 = cos_plus_sin * lateral_velocity - cos_minus_sin * longitudinal_velocity;
    const float velocity_component_w24 = cos_minus_sin * lateral_velocity + cos_plus_sin * longitudinal_velocity;
...
void EstimateRobotPosition(odometry_t *odom, float yaw_angle, wheel_infor_t *wheel_infor, float DeltaTime)
{
    ...
    odom->velocity[X_AXIS] = (cos_plus_sin * wheel_infor[0].WheelSpeed + cos_minus_sin * wheel_infor[1].WheelSpeed - cos_plus_sin * wheel_infor[2].WheelSpeed - cos_minus_sin * wheel_infor[3].WheelSpeed) * WHEEL_RADIUS_SCALE;

    odom->velocity[Y_AXIS] = (-cos_minus_sin * wheel_infor[0].WheelSpeed + cos_plus_sin * wheel_infor[1].WheelSpeed + cos_minus_sin * wheel_infor[2].WheelSpeed - cos_plus_sin * wheel_infor[3].WheelSpeed) * WHEEL_RADIUS_SCALE;

    odom->yaw_rate = WHEEL_RADIUS_SCALE * (1 / YAW_COEFFICIENT) * (wheel_infor[0].WheelSpeed + wheel_infor[1].WheelSpeed + wheel_infor[2].WheelSpeed + wheel_infor[3].WheelSpeed);
