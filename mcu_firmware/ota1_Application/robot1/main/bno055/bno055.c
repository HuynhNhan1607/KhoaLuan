...
esp_err_t bno055_set_default_conf(bno055_config_t *p_bno_conf)
{
    ...
    p_bno_conf->sda_io_num = SDA_PIN;               // GPIO number for I2C sda signal 25
    p_bno_conf->sda_pullup_en = GPIO_PULLUP_ENABLE; // Internal GPIO pull mode for I2C sda signal
    p_bno_conf->scl_io_num = SCL_PIN;               // GPIO number for I2C scl signal 26
    ...
    p_bno_conf->timeout = 31;                       // 10ms in 80 MHz ticks, should be < 0xFFFFF
...
esp_err_t bno055_open(i2c_number_t i2c_num, bno055_config_t *p_bno_conf, bno055_opmode_t mode)
{
    ...
    ESP_LOGE(TAG, "i2c_set_timeout() returned 0x%02X", err);
    if (err != ESP_OK)
        return err;
